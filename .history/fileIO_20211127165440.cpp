...
void FileIO::encodeFile(string desFileName,map<char, string> charCode,map<char, long long> charFreq,int writeMode){
    ...
    for(iter = charCode.rbegin(); iter != charCode.rend(); iter++){
        charInfo[(int)iter->first + 128].code = iter->second;
    }
    //写文件的内容
    ifstream fin(sourceFileName,ios::binary);
    int bufferLength = 0;
    ...
    while(!fin.eof()){
        fin.read(&buffer, sizeof(char));
        for (int strIdx = 0; strIdx < charInfo[(int)buffer+128].code.length();strIdx++){
            bufferbit <<= 1;
            bufferbit |= (charInfo[(int)buffer+128].code[strIdx] == '1');
            bufferLength++;
            //当有一个字符的时候就放进待写数组里
            if(bufferLength==8){
